...
public class AnimController : MonoBehaviour
{
    ...
    [SerializeField] private bool pickUPDone;
    ...
    Coroutine pickUp;
    private void PickupCallback(string status, bool movement)
    {
        if (status.Equals("StartPickup"))
        {
            pickUp = StartCoroutine(PickupIE(status, movement));
        }
    }

    private IEnumerator PickupIE(string status, bool movement)
    {
        movement = false;
        pickUPDone = false;
        animatorPlayer.SetTrigger("Pickup");
        twoBoneIKConstraintPlayerForHandPickup.weight = 1;
        yield return new WaitUntil(() => pickUPDone == true);
        twoBoneIKConstraintPlayerForHandPickup.weight = 0;
        movement = true;
    }
